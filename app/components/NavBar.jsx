...
const NavBar = () => {
  ...
  return (
    <div
      className={`${
        scroll
          ? "bg-cheeseCake dark:bg-[#3F2E3E]"
          : "bg-transparent dark:bg-transparent"
      } fixed top-0 bg-cheeseCake  dark:text-white text-brown font-bold px-4 flex items-center justify-between py-6 w-full z-50 text-sm`}
    >
      {/* DESKTOP NAVIGATION */}
      <nav className=" hidden lg:grid grid-cols-12 w-full items-center">
        ...
        <ul className=" space-x-7 col-start-4 col-span-5  items-center flex ">
          {navLinks.map((nav) => (
            <div
              className={`${
                active === nav.title ? " text-red dark:text-black" : " text-brown dark:text-white"
              } cursor-pointer`}
            >
              <p onClick={() => setActive(nav.title)} key={nav.title} href={nav.href}>
                {nav.title}
              </p>
            </div>
          ))}
        ...
        <div
          ...
          className={`${
            scroll ? " block" : "hidden"
          } w-5 h-5 cursor-pointer col-start-12 ml-3  bg-brown rounded-full`}
        />
        {/* {`${
              scroll ? " fill-current" : " fill-transparent"
            }  w-20 h-full`} */}
